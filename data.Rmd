...
knitr::kable(
  ...
  caption = "Classificação dos pesquisadores -- e instituições -- reponsáveis pelos conjuntos de dados pedológicos de ferro do solo.")
```
...
knitr::kable(
  edafology[, -5], format = "html", table.attr = "width='100%'",
    caption = "Classificação dos pesquisadores -- e instituições -- reponsáveis pelos conjuntos de dados edafológicos de ferro do solo.")
```
...
A primeira figura mostra o número de registros (amostras de horizontes e camadas) em cada uma das unidades da federação. `r states$uf[which.max(states$n)]` concentra o maior número de registros até agora (`r max(states$n)`), enquanto `r states$uf[which.min(states$n)]` possui o menor número de registros (`r min(states$n)`) no repositório de dados de ferro do solo.

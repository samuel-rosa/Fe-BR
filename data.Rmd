...
pedology <- lapply(pedology, function (x) {
  ...
  data.frame(Nome = name, Instituição = inst, uf = uf, Contribuição = number, n = sum(x$Contribuição))
})
pedology <- rank(do.call(rbind, pedology)[, -3])
knitr::kable(
  pedology[, -5], format = "html", table.attr = "width='100%'", 
  caption = "Classificação dos pesquisadores -- e instituições -- reponsáveis pelos conjuntos de dados pedológicos de ferro do solo. As contribuições podem ser descarregadas clicando no respectivo número de registros.")
```
...
edafology <- rank(do.call(rbind, edafology)[, -3])
knitr::kable(
  edafology[, -5], format = "html", table.attr = "width='100%'",
    caption = "Classificação dos pesquisadores -- e instituições -- reponsáveis pelos conjuntos de dados edafológicos de ferro do solo. As contribuições podem ser descarregadas clicando no respectivo número de registros.")
```

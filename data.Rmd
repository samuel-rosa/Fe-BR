...
A figura abaixo mostra, para cada uma das unidades da federação, o número de registros -- dados de horizontes 
e camadas, inclusive repetições de laboratório -- que processamos até agora. Entre em [contato](contact.html) 
conosco caso você -- ou alguém que você conhece -- tenha disponibilidade para ajudar no processamento dos
dado do seu estado.

<!--
`r states$uf[which.max(states$n)]` concentra o maior número de registros até agora (`r max(states$n)`), 
enquanto `r states$uf[which.min(states$n)]` possui o menor número de registros (`r min(states$n)`) no 
repositório de dados de ferro do solo. 
